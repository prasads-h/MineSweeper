(function( $ ){
	...
	function MineSweeper( selector, columnCount, rowCount, bombCount ){
		...
		/*
			binding tapHold event to mark the bomb as detected
		*/
		this.table.on("taphold", function(event){
			self.toggleCaution(jQuery(event.target));
		});
	...
	MineSweeper.prototype.pauseGame = function() {
		this.isPaused = true;
		this.clearInterval();
	}

	MineSweeper.prototype.clearInterval = function() {
		clearInterval(this.TimerId);
	}

	MineSweeper.prototype.resumeGame = function() {
		this.isPaused = false;
		this.TimerId = this.setTimerInterval();
	}
	
	MineSweeper.prototype.toHHMMSS = function(secs) {
